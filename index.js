...
function startQuiz() {
  ...
  $('.startQuiz').on('click', '.startBtn', function (event) {
    ...
    $('.questionBox').show();
    $('.questionBox').prepend(startQuestion());
  });
}
...
function renderQuestion() {
  ...
  <form>
   <fieldset>
     <legend class="questionText">${renderStoreQuestion()}</legend>
      <label class="sizeMe" for="${renderStoreAnswers()[0]}">
      <input class="js-radio" type="radio" id="${renderStoreAnswers()[0]}" value="${renderStoreAnswers()[0]}" name="answer" required>
       <span>${renderStoreAnswers()[0]}</span>
      </label>
      <label class="sizeMe" for="${renderStoreAnswers()[1]}">
      <input class="js-radio" type="radio" id="${renderStoreAnswers()[1]}" value="${renderStoreAnswers()[1]}" name="answer" required>
       <span>${renderStoreAnswers()[1]}</span>
     </label>
     <label class="sizeMe" for="${renderStoreAnswers()[2]}">
     <input class="js-radio" type="radio" id="${renderStoreAnswers()[2]}" value="${renderStoreAnswers()[2]}" name="answer" required>
      <span>${renderStoreAnswers()[2]}</span>
    </label>
    <label class="sizeMe" for="${renderStoreAnswers()[3]}">
    <input class="js-radio" type="radio" id="${renderStoreAnswers()[3]}" value="${renderStoreAnswers()[3]}" name="answer" required>
     <span>${renderStoreAnswers()[3]}</span>
   </label>
   <label class="sizeMe" for="${renderStoreAnswers()[4]}">
   <input class="js-radio" type="radio" id="${renderStoreAnswers()[4]}" value="${renderStoreAnswers()[4]}" name="answer" required>
     <span>${renderStoreAnswers()[4]}</span>
     </label>
      <div class= "buttonContainer"><button type="submit" class="submitButton button"> Submit</button > </div>
  </fieldset>
</form>
  `)
}
...
function nextQuestion() {
  $('.duneContainer').on('click', '.nextButton', function (event) {
    ...
    $('.questionBox form').replaceWith(startQuestion());
  });
}
...
function wrongAnswer() {
  $('.response').html(
    `<div class="wrongAn"><h3>Wrong!</h3></div>
          <p class="sizeMe wrongAnswer">Frank Herbert wanted:</p>
...
function getQuestionPlusAnswers() {
  return STORE[questionNumber];
}

function renderStoreQuestion() {
  let questionStore = getQuestionPlusAnswers().question;
  return questionStore;
}

function renderStoreAnswers() {
  let eveAnswers = getQuestionPlusAnswers().answers;
  return eveAnswers;
}
